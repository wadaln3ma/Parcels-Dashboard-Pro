...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // auto-switch to Carto Dark when user prefers carto in dark mode
    const current = basemap;
    if (dark && current === 'carto') setBasemapLayer('carto-dark' as any);
    if (!dark && current === 'carto-dark') setBasemapLayer('carto' as any);
  }, [dark]);
  ...
  function toggleLayer(ref: any, show: boolean) {
    if (!mapRef.current) return;
    const layer = ref?.current;
    if (!layer) return; // layer not loaded yet; the loader will respect showRoads/showZoning later
    if (show) {
      try { layer.addTo(mapRef.current); } catch {}
    } else {
  ...
  async function downloadPNG() {
    ...
    const canUseCurrent = !!(m._currentBase && (m._currentBase.options?.crossOrigin || m._currentBase.getLayers)); // groups are OK
    if (!canUseCurrent) {
    ...
    const canvas = await html2canvas(mapDivRef.current, { useCORS: true, allowTaint: true, backgroundColor: null, logging: false as any });
  ...
  function getBasemapLayer(kind: 'osm'|'carto'|'carto-dark'|'esri'|'hybrid'): L.Layer {
    ...
    if (kind === 'hybrid') {
      ...
      // Optional transportation ref layer could be added similarly
  ...
  function setBasemapLayer(kind: 'osm'|'carto'|'carto-dark'|'esri'|'hybrid') {
    ...
    m._currentBase && m.removeLayer(m._currentBase);
  ...
  return (
    <main className={'max-w-6xl mx-auto p-4'}>
        ...
        <div ref={overlayRef} className="pointer-events-none absolute top-3 left-3 right-3 md:left-4 md:right-auto z-[500] space-y-2">
          <div className="pointer-events-auto glass rounded-2xl p-3 md:w-[560px]">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button onClick={async()=>{ setLocating(true); try{ await new Promise<void>((resolve,reject)=>{ mapRef.current?.locate({setView:true,maxZoom:16}); mapRef.current?.once('locationfound', ()=>resolve()); mapRef.current?.once('locationerror', ()=>reject()); }); }catch{} finally{ setLocating(false); } }} className="px-3 py-2 rounded border bg-white dark:bg-slate-800" title="Locate me">
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {(tableFeatures.features||[]).map((f:any)=> (
                    <tr key={f.properties?.id} onClick={()=>zoomToRow(f)} className="cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800/50" role="button" title="Click to zoom">
      ...
      <p className="text-slate-500 text-sm mt-2">Tip: Click a parcel to select; then buffer or upload/draw an AOI.</p>
    </main>
  );
}
